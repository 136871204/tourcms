...
class UserControl extends AuthControl {
	...
	public function index() {
        ...
        //TODO:审核功能之后在调整
        if($state==0){
            $this -> data = $Model-> order("uid ASC") -> where("username<>'$WEBMASTER' AND user_state = $state") -> all();
        }else{
            $this -> data = $Model-> order("uid ASC") -> where("username<>'$WEBMASTER' AND user_state = 1") -> all();
        }
        $this->state=$state;
