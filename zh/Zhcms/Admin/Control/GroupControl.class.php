...
class GroupControl extends AuthControl {
	...
	public function add() {
		if (IS_POST) {
			...
			if ($aid = $role -> addRole($_POST)) {
				$this -> success(L('admin_group_control_add_success'));
			}
		} else {
			$this -> display();
		}
	...
	public function updateCache() {
		...
		if ($RoleModel -> updateCache()) {
			$this -> success(L('admin_group_control_update_cache_success'));
		} else {
	...
	public function edit() {
		if (IS_POST) {
			...
			if ($Model -> editRole($_POST)) {
				$this -> success(L('admin_group_control_edit_success'));
			} else {
	...
	public function del() {
		...
		if ($rid) {
			...
			//用户组关联表
			$Model -> delRole($rid);
			$this -> success(L('admin_group_control_del_success'));
		}
	}
