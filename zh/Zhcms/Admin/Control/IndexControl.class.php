...
class IndexControl extends AuthControl{
	...
	public function index() {
       ...
       $favoriteMenu = cache($_SESSION['uid'], false, MENU_CACHE_PATH);
       $topMenu=$nodeModel->change_title($topMenu);
       
       $favoriteMenu=$nodeModel->change_title($favoriteMenu);
	...
	public function getChildMenu() {
	   ...
	   $pid = Q('pid', 0, 'intval');
       if (empty($pid)) {
			$this -> ajax(L('admin_index_control_get_child_menu_error1'));
		}
        //超级管理员获得所有菜单
        ...
        $MenuData=$nodeModel->change_title($MenuData);
        //去掉隐藏的菜单
		$showMenuData = array();
	...
	public function setFavorite() {
		if (IS_POST) {
			...
			$this -> success(L('admin_index_control_set_favorite_success1'));
		} else {
		  //echo MENU_CACHE_PATH;die;
			if (!is_writable(MENU_CACHE_PATH)) {
				$this -> error(MENU_CACHE_PATH . L('admin_index_control_set_favorite_error1'));
			}
			$nodeModel = V('node');
			$pre = C('DB_PREFIX');
            ...
            $nodeKModel=K('Node');
            $nodeData=$nodeKModel->change_title($nodeData);
