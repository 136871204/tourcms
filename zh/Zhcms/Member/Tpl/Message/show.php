...
<article class="center-block main">
    ...
    <section class="article">
        <form onsubmit="return zh_submit(this,'{|U:'show',array('from_uid'=>$_GET['from_uid'])}')" action="{|U:'reply'}">
            <input type="hidden" name="to_uid" value="{$zh.get.from_uid}"/>
            <table>
                <tr>
                    <td>
                ...
                <tr>
                    <td colspan="2">
                        <input type="submit" value="返事" class="zh-success"/>
                    </td>
                </tr>
            </table>
        </form>
                    ...
                    <div class="user">
                        <div class="icon">
                            <a href="__WEB__?{$d.from_uid}" target="_blank">
                                <img src="{$d['icon']|default:'__ROOT__/data/image/user/100.png'}" onmouseover="user.show(this,{$zh.session.uid})" style="width:50px;"/>
                            </a>
                        </div>
                        <p>{$d.content}</p>
                    </div>
