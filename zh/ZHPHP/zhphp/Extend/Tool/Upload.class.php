...
class Upload
{
     ...
     * </code>
     */
    public function uploadFile($fieldName = null)
    {
    ...
    private function save($file)
    {
        ...
        if (!$is_img) {
            ...
            if (C("UPLOAD_IMG_RESIZE_ON") && ($imgInfo[0] > C("UPLOAD_IMG_MAX_WIDTH") || $imgInfo[1] > C("UPLOAD_IMG_MAX_HEIGHT"))) {
                $img->thumb($filePath, $uploadFileName, C("UPLOAD_IMG_MAX_WIDTH"), C("UPLOAD_IMG_MAX_HEIGHT"), C("thumb_type"), $this->path);
            }
            ...
            if ($this->thumbOn) {
                $args = array();
                if(!empty($this->thumb)){
                    foreach($this->thumb as $tk=>$tv){
                        
                        $tv[0]=$tv[0].$uploadFileName;
                        $args = array();
                        if (empty($tv)) {
                            array_unshift($args, $filePath);
                        } else {
                            array_unshift($args, $filePath);
                            $args = array_merge($args, $tv);
                        }
                        //p($args);
