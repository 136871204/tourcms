...
//		self::success($html);
	...
	static private function backup_structure() {
		$sql = "<?php if(!defined('ZHPHP_PATH'))EXIT;\n";
		$db = M();
		$db_prefix = C("DB_PREFIX");
	...
	static private function write_backup_data($table, $data, $current_row) {
		...
		file_put_contents(self::$dir . "/{$table}_bk_{$fid}.php", "<?php if(!defined('ZHPHP_PATH'))EXIT;\n{$data}");
