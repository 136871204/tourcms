//当前文件只能加载一次
zh_uploadify_js = true;
var zh_uploadify_options = {
//    removeTimeout   : 11//提示框消失时间
    ...
    uploader: ZHPHP_CONTROL + "&m=zh_uploadify"//上传处理php
    ,
    ...
    onSWFReady: function () {//swfupload准备好
        ...
        this.queueData.uploadsSuccessful = ZHPHP_UPLOAD_TOTAL;
        //修改全局环境量
        this.setStats({
    ...
    onUploadSuccess: function (file, data, response) {
        ...
        //第几个上传文件，会受全局变量ZHPHP_UPLOAD_TOTAL的影响
        ...
        html += "</li>";
        div.append(html);
        if (typeof(zh_upload) == 'function') {
            zh_upload(file, data, response);
        }
    }
};
...
$(".delUploadFile").live("click", function () {
    ...
    $.post(ZHPHP_CONTROL + "&m=zh_uploadify_del", {
        file: delFiles
    }, function (data) {
        if (data == 1) {
            upload.setStats({
