...
class ModelFieldModel extends Model {
    ...
    {
        C('language','Model'.DS.'ModelField'.DS.$_SESSION['language']);
    }
	...
	public function delField() {
		$fid = Q('fid');
		if (!$fid) {
			$this -> error = L('model_model_field_del_field_error1');
			return false;
		}
		//获得字段信息
		...
		if ($this -> fieldExists($field['field_name'], $field['table_name'])) {
			...
			if (!$this -> exe($sql)) {
				$this -> error = L('model_model_field_del_field_error2');
				return false;
			}
		}
		//删除字段表记录
	...
	public function addField($data) {
		...
		if (!method_exists($this, $method)) {
			$this -> error = L('model_model_field_add_field_error1');
			return false;
		}
		...
		if ($this -> create($data)) {
			...
			if ($this -> alterTableField()) {
				if ($this -> add()) {
					...
					$this -> error = L('model_model_field_add_field_error2');
				}
			}
		}
	...
	public function editField($data) {
		$method = $data['field_type'];
		if (!method_exists($this, $method)) {
			$this -> error = L('model_model_field_edit_field_error1');
			return false;
		}
		$data = $this -> $method($data, null);
		if ($this -> create($data)) {
			...
			if ($state) {
				...
				$this -> error = L('model_model_field_edit_field_error2');
			}
		}
	...
	public function alterTableField() {
		...
		if (!$field_name) {
			$this -> error = L('model_model_field_alter_table_field_error1');
			return false;
		}
		//SQL
		...
		if ( M() -> exe($sql)) {
			...
			$this -> error = L('model_model_field_alter_table_field_error2');
			return false;
		}
	}
	...
	public function updateCache($mid=null) {
		...
		if (!isset($this -> _model[$mid])) {
			$this -> error = L('model_model_field_update_cache_error1');
			return false;
		}
		//查找当模型所有字段信息
		$ModelField = M("field");
		...
		if (!cache($mid, $cacheData, FIELD_CACHE_PATH)) {
			$this -> error = L('model_model_field_update_cache_error2');
			return false;
		} else {
	...
	public function updateTableFieldCache() {
		$dirArr = array('temp/Zhcms/Content/Table', 'temp/Zhcms/Index/Table');
		foreach ($dirArr as $dir) {
			if (is_dir($dir)) {
				if (!is_writable($dir)) {
					$this -> error = L('model_model_field_update_table_field_cache_error1');
					return false;
				} else {
					if (Dir::del($dir)) {
						$this -> error = L('model_model_field_update_table_field_cache_error2');
						return false;
					}
				}
			}
    ...
    //外部数据
	private function exterior($fieldInfo, $value) {
		return $fieldInfo;
	}

    //ツリー選択
	private function treeselect($fieldInfo, $value) {
		return $fieldInfo;
	}
