...
class ContentInputModel {
	...
	public function get(array $InsertData) {
		...
		foreach ($this->_field as $field => $fieldInfo) {
			...
			if (isset($set['maxlength']) && !empty($set['maxlength'])) {
				...
				if ($maxlength && $maxlength > $minlength) {
					$validateRule[] = array($field, "minlen:$minlength", $fieldInfo['title'] . " {$minlength}文字以上にしてください", $validateOccasion, 3);
					$validateRule[] = array($field, "maxlen:$maxlength", $fieldInfo['title'] . " {$maxlength}文字以内にしてください", $validateOccasion, 3);
				}
			}
			//验证规则
			if (isset($fieldInfo['validate']) && !empty($fieldInfo['validate'])) {
				$regexp = $fieldInfo['validate'];
				$error = empty($fieldInfo['error']) ? $fieldInfo['title'] . ' 入力エラー' : $set['error'];
				$validateRule[] = array($field, "regexp:$regexp", $error, $validateOccasion, 3);
			}
			//必须输入验证
			if (isset($fieldInfo['required']) && (int)$fieldInfo['required'] == 1) {
				$validateRule[] = array($field, "nonull", $fieldInfo['title'] . ' 必須', $validateOccasion, 3);
			}
		...
		if (empty($InsertData['thumb'])) {
			if (false !== $k = array_search('画像', $flag)) {
				unset($flag[$k]);
			}
		} else {
			$flag[] = '画像';
	...
	private function thumb($fieldInfo, $value) {
       ...
       echo $contentStr."<br/>";
       preg_match_all("/(src)=([\"|']?)([^ \"'>]+\.(gif|jpg|jpeg|bmp|png))\\2/i", $contentStr, $matches);
       preg_match_all('/(src)=([\'"]?)([^\'">]+\.(jpg|jpeg|png|gif))\2/is', $contentStr, $matchData, PREG_PATTERN_ORDER);
       p($matches);
       p($matchData);
       echo 'adfs';die;*/
		//提取内容第n张图为缩略图
	...
	private function images($fieldInfo, $value) {
	   //echo 'aaaa';die;
	  // p($fieldInfo);
	    $name_sort = $fieldInfo['set']['name_sort'];
        //echo p($value);die;
        if($name_sort==2){
