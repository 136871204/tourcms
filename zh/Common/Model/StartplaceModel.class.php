...
class StartplaceModel extends ViewModel {
    ...
    //出发城市
    function getStartCityNameShow($id)
    {
        $all_startplace=cache('all_startplace');
        if(empty($all_startplace)){
            $sql = "SELECT cityname FROM ".$this->tableFull." WHERE id='$id'";
    	   return  $this->getOne($sql,'cityname');
        }else{
           foreach($all_startplace as $place){
                if($place['id']==$id){
    ...
    function updateCache()
    {
		...
		$topsql="select * from ".$db_prefix."startplace where isopen=1 and pid!=0 order by displayorder asc ";
        $startplace=M()->query($topsql);
        
        $allsql="select * from ".$db_prefix."startplace  ";
        $allstartplace=M()->query($allsql);
        
        cache('all_startplace',$allstartplace);
        
        return cache('top_startplace',$startplace);
    }
