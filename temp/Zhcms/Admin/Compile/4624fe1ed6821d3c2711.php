...
echo "";
    ...
    <div class="menu_list" style="clear: both;">
        <ul>
                <li><a href="<?php echo U('attr');?>" class="action"><?php echo $currentKindName;?>列表</a></li>
                <li><a href="<?php echo U('attr_add',array('webid'=>$_GET['webid']));?>">添加<?php echo $currentKindName;?></a></li>
        </ul>
    </div>
...
if($_index_info>=$_total_info):$zh["list"]["info"]["last"]=true;endif;?>
            ...
            <tr  align="center" class="<?php echo $info['level'];?>" id="<?php echo $info['level'];?>_<?php echo $info['id'];?>" >
                ...
                <td  >
                    <a href="<?php echo U('attr_add',array('pid'=>$info['id'],'webid'=>$_GET['webid']));?>">
						添加子目的地
				    </a>|
                    <a href="<?php echo U('attr_edit',array('id'=>$info['id'],'webid'=>$_GET['webid']));?>">
				        修改
				    </a>|
                    <a href="javascript:if(confirm('确定删除吗？'))zh_ajax('<?php echo U(attr_del);?>',{id:<?php echo $info['id'];?>,webid:<?php echo $_GET['webid'];?>})">
				        删除
				    </a>
                </td>
            </tr>
